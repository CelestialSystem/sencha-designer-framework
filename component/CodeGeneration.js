...
// Touch specific template helpers
...
templates.add('indexhtml', Ext4.create('Ext.XTemplate',
    ...
    '{[this.newline()]}',
    '<html manifest="">', '{[this.newline()]}',
    '<head>', '{[this.newline()]}',
        '{[this.indent(1)]}<meta http-equiv="X-UA-Compatible" content="IE=edge">', '{[this.newline()]}',
        '{[this.indent(1)]}<meta charset="UTF-8">', '{[this.newline()]}',
        '{[this.indent(1)]}<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">', '{[this.newline()]}',
        '{[this.newline()]}',
        ...
        '{[this.indent(1)]}<!-- The line below must be kept intact for Sencha Cmd to build your application -->', '{[this.newline()]}',
...
templates.add('indexjs', Ext4.create('Ext.XTemplate',
    ...
    'Ext.application({','{[this.newline()]}',
        '<tpl for="refs">',
            '{[this.indent(1)]}{[this.encodeConfigName(values.name)]}: {[ this.encodeRef(values.value, 1) ]},', '{[this.newline()]}',
            '{[this.newline()]}',
        '</tpl>',
        '<tpl for="configs">',
...
templates.add('createInitialView', Ext4.create('Ext.XTemplate',
    // '<tpl if="initialViewClassName">',
    'Ext.create(\'{qualifiedClassName}\', {fullscreen: true});',
    // '</tpl>'
    templateHelpers
));
templates.get('createInitialView').cfg = {};
...
templates.add('controllerclass', Ext4.create('Ext.XTemplate',
    ...
    'Ext.define(\'{qualifiedClassName}\', {', '{[this.newline()]}',
        ...
        '<tpl if="processCfg">',
            ...
            '{[this.indent(1)]}','}',
        '</tpl>',
        '<tpl if="beforeFilters.length">',
            ',', '{[this.newline()]}', '{[this.newline()]}',
            '{[this.indent(1)]}before: {',
            '<tpl for="beforeFilters">',
                '{[this.newline()]}',
                '{[this.encodeProperty(values, "routeAction", "fn", 2)]}',
...
templates.add('controller.control', Ext4.create('Ext.XTemplate',
    ...
    '<tpl for="snippets">',
        '{[this.indent(1)]}', '"{query}": {', '{[this.newline()]}', '{[this.trackOpen(values, false)]}',
        '<tpl for="listeners">',
            '{[this.indent(2)]}', '{event}: ', "'{fn}'", '{[this.endingComma(xcount, xindex)]}', '{[this.newline()]}', '{[this.trackLine(values)]}',
        '</tpl>',
        '{[this.indent(1)]}', '}', '{[this.endingComma(xcount, xindex)]}', '{[this.newline()]}', '{[this.trackClose(values, false)]}',
    '</tpl>',
    '}',
    templateHelpers
));
...
templates.add('controller.applisteners', Ext4.create('Ext.XTemplate',
    ...
    '<tpl for="events">',
        '{[this.indent(3)]}','{ event: \'{name}\', ', '{[this.trackOpen(values, false)]}',
        '<tpl for="listeners">',
            '{[this.encodeSimpleConfig(values.action, null, true)]}', '{[this.trackLine(values)]}',
        '</tpl>',
    ...
    'Ext.define(\'{qualifiedClassName}\', {', '{[this.newline()]}',
        ...
        ',', '{[this.newline()]}{[this.newline()]}',
            '<tpl for="configs">',
                '<tpl if="xindex !== 1">,</tpl>',
                '{[this.trackLine(values, true)]}',
                '{[this.newline()]}{[this.indent(1)]}{[this.encodeConfigName(values.name)]}: {[ this.encodeConfig(values, 1) ]}',
            '</tpl>',
            '<tpl if="configs.length && refs.length">,</tpl>',
            '<tpl for="refs">',
                '<tpl if="xindex !== 1">,</tpl>',
                '{[this.newline()]}',
                '{[this.trackOpen(values, true)]}',
        ...
        '<tpl if="members.length">',
            ...
            '<tpl for="members">',
                '{[this.trackOpen(values, true)]}',
                '{[this.newline()]}', '{[this.encodeMember(values)]}',
                '{[this.trackClose(values, true)]}',
            '</tpl>',
        '</tpl>',
        '<tpl if="methods.length">',',','{[this.newline()]}','{[this.newline()]}',
...
Ext4.override(xds.component.Definition, {
    ...
    /**
     * In Touch, certain Function configs are allowed to go in the config block in the generated code
     * (e.g. onItemDisclosure in Ext.dataview.List)
     * These are marked by 'applyToConfig' on the config instance
     */
    shouldApplyFnToConfig: function(fnName) {
        var noSameNameCustomConfig =  this.hasCfg(fnName) && !this.hasCustomCfg(fnName),
            functionConfig;

        if (noSameNameCustomConfig) {
            functionConfig = this.getCfg(fnName).getAlternateOfType('function');

            if (functionConfig && functionConfig.getApplyToConfig) {
                return functionConfig.getApplyToConfig();
            }
        }
    ...
    getEventHandlerMethodConfigs: function() {
        var me = this,
            ...
            directEventBindings = me.query('> basiceventbinding');
        ...
        Ext4.Array.each(eventBindings, function(eventBinding) {
            var cfg = eventBinding.getEventBindingConfig(),
                evName = eventBinding.getCfgValue('name'),
                // Check if these events are not hoistable i.e they mandate we apply listener to the child since delegate doesnt work for these
                nonHoistableEvent = me.nonHoistableEvents && me.nonHoistableEvents.contains(evName),
                cfgFly, hasMemberCfg;

            if (cfg) {
                cfgFly = me.flyCfg(cfg.fnName);
                // configs are applied as members and a config of this name exists as a member config
                hasMemberCfg = me.areCfgsAppliedAsMembers() && cfgFly && cfgFly.includeInCodeGen;

                // this function is a direct eventbings or hoistableEvent that doesn't share config name with a member config
                // and isn't marked to be applied to the classes config
                if ((directEventBindings.contains(eventBinding) || !nonHoistableEvent) &&
                    !hasMemberCfg && !this.shouldApplyFnToConfig(cfg.fnName)) {
